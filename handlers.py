...
from database.core import insert_user, get_users, on_message_received
...
async def cmd_photo(message: types.Message):
    # Проверяем, прикреплено ли фото к сообщению
    ...
    # Теперь можно работать с загруженным файлом
...
async def cmd_weather(message: types.Message):
    ...
    # Отправляем информацию о температуре пользователю
...
def reg_handlers(dp: Dispatcher):
    ...
    dp.message.register(on_message_received)